...
public class SearchProductDAO extends BaseDAO {
        ...
        public ArrayList<HangHoa> getDsHangHoa(String searchText, int pageNumber) {
        	...
        	ArrayList<HangHoa> returnedList = new ArrayList<HangHoa>();
        	                ...
        	                String sql = "SELECT * FROM (SELECT RowNum = ROW_NUMBER() OVER (ORDER BY mahh), * FROM HangHoa WHERE MaHH LIKE ? OR TenHH LIKE ? OR donvitinh LIKE ? OR ghichu LIKE ? OR DonGiaThamKhao = ?) AS tempTable WHERE RowNum > (? * (? - 1)) AND RowNum <= (? * (? - 1)) + ? ORDER BY mahh";                
